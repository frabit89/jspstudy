...
public class BoardDaoImpl implements BoardDao {
  ...
  //MyBatis의 주요 설정 정보를 가지고 있으며, 데이터베이스와의 통신을 처리하기 위해 SqlSession 객체를 생성하는 역할
  ...
  public int updateBoard(BoardDto board) {
    ...
    int updateCount = sqlSession.update("com.gdu.prj.dto.BoardDto.updateBoard", board);
    if(updateCount == 1) {
      sqlSession.commit();
    }
    sqlSession.close();
  ...
  public int deleteBoard(int board_no) {
    SqlSession sqlSession = factory.openSession(false);
    int deleteCount = sqlSession.delete("com.gdu.prj.dto.BoardDto.deleteBoard", board_no);
    if(deleteCount == 1) {
      sqlSession.commit();
    }
    sqlSession.close();
